...
void Player::update(int deltaTime) {
    ...
    static bool flank = false;
    if (Game::instance().getKey(GLFW_KEY_1) && !flank) {
        UIManager::instance().createFloatingPoints(
            "123",
            glm::vec2(position + glm::vec2(TILE_SIZE * 5.f / 2.f, 0)),
            glm::vec3(1.f, 0.f, 0.f),
            TILE_SIZE * 3 / 2, 1000.f
        );
        flank = true;
    }
